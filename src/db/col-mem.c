 ...
 * Whitespace Separated Values i.e. output from standard commands
 *
 * @param db
    ...
    size_t value_max_length)
{
    ...
    long file_offset_end = field_index == db->field_count - 1 ? db->line_indices[record_index + 2] : db->line_indices[record_index + 1] + fields->fieldStart[field_index + 1];
