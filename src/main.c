...
char *shorthand_helper(char **buffer, int *flags, const char **format_val);
...
int main(int argc, char *argv[])
{
    ...
    while (argi < argc)
    {
        ...
        else if (strcmp(arg, "-S") == 0)
        {
            if (argi + 1 >= argc)
            {
                fprintf(stderr, "Expected SELECT clause to be specified after -S\n");
                printUsage(argv[0], stderr);
                exit(-1);
            }
            ...
            sprintf(ptr, " SELECT %s", argv[++argi]);
        }
        else if (strcmp(arg, "-W") == 0)
        {
            if (argi + 1 >= argc)
            {
                fprintf(stderr, "Expected WHERE clause to be specified after -W\n");
                printUsage(argv[0], stderr);
                exit(-1);
            }
            ...
            sprintf(ptr, " WHERE %s", argv[++argi]);
        }
        else if (strcmp(arg, "-O") == 0)
        {
            if (argi + 1 >= argc)
            {
                fprintf(stderr, "Expected ORDER BY clause to be specified after -W\n");
                printUsage(argv[0], stderr);
                exit(-1);
            }
            ...
            sprintf(ptr, " ORDER BY %s", argv[++argi]);
        }
...
// Used by -S, -W, and -O shorthand command line options
char *shorthand_helper(char **buffer, int *flags, const char **format_val)
{
    if (*format_val == NULL)
    {
