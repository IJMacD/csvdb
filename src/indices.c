 ...
 */
enum IndexSearchResult indexPrimarySeek (struct DB *db, enum Operator predicate_op, const char *predicate_value, struct RowList * row_list, int limit) {
    return indexUniqueSeek(db, FIELD_ROW_INDEX, predicate_op, predicate_value, row_list, limit);
}

/**
 * Pretty much duplicated from indexSeek but telling the underlying DB VFS that the index is unique
 * Also: only need to do one index search, rather than two.
 */
enum IndexSearchResult indexUniqueSeek (struct DB *index_db, int rowid_column, enum Operator predicate_op, const char *predicate_value, struct RowList * row_list, int limit) {
 ...
 */
enum IndexSearchResult indexScan (struct DB *index_db, int rowid_column, struct RowList * row_list, int limit) {
    // (inclusive)
    int lower_bound = 0;
    // (exclusive)
 ...
 * @return number of matched rows; RESULT_NO_INDEX if index does not exist
 */
enum IndexSearchResult indexSeek (struct DB *index_db, int rowid_column, enum Operator predicate_op, const char *predicate_value, struct RowList * row_list, int limit) {
    // (inclusive)
    int lower_bound;
    // (exclusive)
    int upper_bound;
    ...
    if (predicate_op == OPERATOR_LIKE) {
        if (value[len - 1] != '%') {
            fprintf(stderr, "Cannot do INDEX SEEK on LIKE with without '%%' at the end\n");
            exit(-1);
        }
        value[len - 1] = '\0';
    }
